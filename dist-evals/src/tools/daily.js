...
async function gatherVaultData(vaultRoot, targetDate) {
    const items = [];
    // Gather tasks
...
async function generateDailySummary(targetDate, items, context) {
    ...
    const tasks = items.filter(i => i.type === 'task').sort((a, b) => {
        ...
        const pA = priority[a.metadata?.priority || 'medium'];
        const pB = priority[b.metadata?.priority || 'medium'];
        return pA - pB;
    });
...
`;
    ...
    if (tasks.length > 0) {
        ...
        for (const t of tasks) {
            ...
            prompt += `- ${priorityEmoji[t.metadata?.priority || 'medium']} ${t.title}${t.metadata?.dueDate ? ` (due: ${t.metadata.dueDate})` : ''}\n`;
        }
        prompt += '\n';
    }
...
async function saveDailyFile(vaultRoot, date, summary) {
...
// ─────────────────────────────────────────────────────────────────────────────
async function generateDailyForDate(date, input, context) {
