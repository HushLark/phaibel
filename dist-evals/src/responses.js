...
import { getUserName, getUserGender, getAgentName, getPersonalityId } from './state/manager.js';
import { getPersonality } from './personalities.js';
// ── Cached values for sync substitution ────────────────────────────────────
let cachedName = 'friend';
let cachedAgentName = 'Agent';
let cachedPersonalityId = 'butler';
let cachedGender = 'other';
...
function randomHonorific() {
    ...
    if (pool.length === 0)
        return cachedName; // executive personality
 ...
 */
export async function initResponseName() {
    if (cacheLoaded)
        return;
    try {
 ...
 */
export async function refreshResponseCache() {
    cacheLoaded = false;
 ...
 */
export function getResponseWith(key, replacements) {
    const options = responses[key];
    if (!options || options.length === 0) {
        return '';
    }
