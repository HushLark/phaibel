...
import { SecretsSchema, ConfigSchema, LLM_CAPABILITIES } from './schemas/index.js';
...
const CAPABILITY_PREFERRED_PROVIDERS = {
...
async function ensureVaultConfigDir() {
...
export async function getCapabilityModel(capability) {
...
export async function getEffectiveConfig() {
    const result = {};
    await Promise.all(LLM_CAPABILITIES.map(async (cap) => {
        result[cap] = await getCapabilityModel(cap);
    }));
    return result;
}
export async function setCapabilityModel(capability, provider, model) {
    const config = await loadConfig();
    config.capabilityMapping[capability] = { provider, model };
    await saveConfig(config);
}
export async function removeCapabilityOverride(capability) {
    const config = await loadConfig();
