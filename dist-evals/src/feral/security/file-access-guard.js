...
export class FileAccessDeniedError extends Error {
    constructor(filePath) {
        super(`File access denied: "${filePath}" is outside the vault and ~/.phaibel/. ` +
 ...
 */
function isUnder(filePath, allowedDir) {
 ...
 */
export async function assertPathAllowed(filePath) {
    const resolved = resolveAbsolute(filePath);
    // Always allow ~/.phaibel/
    if (isUnder(resolved, getSystemDir()))
        return;
    // Allow the vault root (if one is active)
    const vaultRoot = await findVaultRoot();
    if (vaultRoot && isUnder(resolved, vaultRoot))
        return;
 ...
 */
export function assertPathAllowedSync(filePath, vaultRoot) {
    const resolved = resolveAbsolute(filePath);
    if (isUnder(resolved, getSystemDir()))
        return;
    if (vaultRoot && isUnder(resolved, vaultRoot))
