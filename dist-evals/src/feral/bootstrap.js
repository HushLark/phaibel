 ...
 */
function getCrossPlatformNodeCodes() {
    return [
        // Flow
        new StartNodeCode(),
        new StopNodeCode(),
 ...
 */
async function getNodeOnlyNodeCodes() {
    const [{ ReadFileNodeCode }, { WriteFileNodeCode }, { WriteToRedisNodeCode }, { SlackBlockBuilderNodeCode }, { SlackPostWebhookNodeCode }, { SlackProcessSlashCommandNodeCode }, { CliCommandNodeCode }, { IntrospectNodeCode }, { PampSendNodeCode }, { PampCheckInboxNodeCode }, { PampShareEntityNodeCode }, { PampAwaitReplyNodeCode }, { ListSchedulerJobsNodeCode }, { ToggleSchedulerJobNodeCode }, { RunSchedulerJobNodeCode }, { McpCallToolNodeCode }, { A2ASendTaskNodeCode }, { FcpProbeNodeCode }, { FcpFetchNodeCode }, { CxfDiscoverNodeCode }, { CxfPullNodeCode }, { CxfPushNodeCode },] = await Promise.all([
 ...
 */
async function getNodeOnlyCatalogSources(mcpTools, a2aAgents) {
    const [{ SlackCatalogSource }, { AgentCatalogSource }, { SystemCatalogSource }, { IntrospectCatalogSource }, { AnalyticsCatalogSource }, { PampCatalogSource }, { McpCatalogSource }, { A2ACatalogSource }, { FcpCatalogSource }, { CxfCatalogSource },] = await Promise.all([
 ...
 */
export async function bootstrapFeral(processSourcesOrOpts = []) {
    const opts = Array.isArray(processSourcesOrOpts)
    ...
    const vaultProcesses = vaultProcessSource.getProcesses();
    // 4c. Load Foundation-level process definitions from (Root)/feral/processes/
    let feralProcessSource = null;
    try {
    ...
    const allSources = [jsonProcessSource, vaultProcessSource, ...processSources];
    if (feralProcessSource)
        allSources.push(feralProcessSource);
