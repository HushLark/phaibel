...
export class PampAwaitReplyNodeCode extends AbstractNodeCode {
    ...
    async process(context) {
        const msgIdTemplate = this.getRequiredConfigValue('message_id');
        const timeout = this.getOptionalConfigValue('timeout_seconds', 60);
        const pollInterval = this.getOptionalConfigValue('poll_interval_seconds', 5);
        const resultPath = this.getOptionalConfigValue('result_context_path', 'pamp_reply');
        const messageId = msgIdTemplate.replace(/\{(\w+)\}/g, (_, key) => String(context.get(key) ?? ''));
        try {
            ...
            while (Date.now() < deadline) {
                ...
                const reply = headers.find((h) => h.chain && h.chain.includes(messageId));
                if (reply) {
