...
export class FindEntityNodeCode extends AbstractNodeCode {
    ...
    async process(context) {
        ...
        const titleKey = this.getRequiredConfigValue('title_context_key', 'title');
        const contextPath = this.getRequiredConfigValue('context_path', 'entity');
        // Bridge config → context: LLMs often put the title in config
        const configTitle = this.getOptionalConfigValue('entity_title');
        const configTitleDirect = this.getOptionalConfigValue('title');
        if (configTitle) {
        ...
        const title = context.get(titleKey);
        if (!title) {
