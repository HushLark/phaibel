...
export class CxfDiscoverNodeCode extends AbstractNodeCode {
    ...
    async process(context) {
        ...
        const resultPath = this.getOptionalConfigValue('result_context_path') ?? 'cxf_schema';
...
function parseVSchemas(cxfText) {
    const types = [];
    ...
    for (let i = 1; i < blocks.length; i++) {
        ...
        const type = { name: '', plural: '', fields: [] };
        for (const line of lines) {
            if (line.startsWith('X-CXF-TYPE-NAME:'))
                type.name = line.slice('X-CXF-TYPE-NAME:'.length).trim();
            else if (line.startsWith('X-CXF-PLURAL:'))
                type.plural = line.slice('X-CXF-PLURAL:'.length).trim();
            else if (line.startsWith('X-CXF-DESCRIPTION:'))
                type.description = line.slice('X-CXF-DESCRIPTION:'.length).trim().replace(/\\n/g, '\n');
