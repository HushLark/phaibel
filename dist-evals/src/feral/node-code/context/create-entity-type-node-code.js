...
export class CreateEntityTypeNodeCode extends AbstractNodeCode {
    ...
    constructor() {
        super('create_entity_type', 'Create Entity Type', 'Uses the LLM to design a field schema, then registers a new entity type.', NodeCodeCategory.DATA);
    }
    async process(context) {
        const rawName = this.getRequiredConfigValue('type_name');
        const rawDesc = this.getRequiredConfigValue('description');
        ...
        const created = context.get('created_entity_types') ?? [];
