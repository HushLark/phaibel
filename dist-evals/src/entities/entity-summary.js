 ...
 */
export async function generateEntitySummary(entityType, title, content, meta) {
    try {
...
${metaSnippet ? `Metadata:\n${metaSnippet}\n` : ''}${content ? `Content:\n${content.slice(0, 500)}` : ''}`;
        const summary = await llm.chat([{ role: 'user', content: prompt }], { maxTokens: 100, temperature: 0.3 });
