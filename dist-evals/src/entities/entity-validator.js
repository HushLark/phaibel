...
function validateField(fieldDef, value) {
    ...
    switch (fieldDef.type) {
        ...
        case 'date':
            if (typeof value !== 'string')
                return `must be a date string (YYYY-MM-DD), got ${typeof value}`;
            if (!DATE_RE.test(value))
                return `must be YYYY-MM-DD format, got "${value}"`;
        ...
        case 'datetime':
            if (typeof value !== 'string')
                return `must be a datetime string, got ${typeof value}`;
            if (!DATETIME_RE.test(value))
                return `must be ISO datetime format (YYYY-MM-DDTHH:MM...), got "${value}"`;
            if (isNaN(new Date(value).getTime()))
                return `invalid datetime: "${value}"`;
            break;
        case 'duration':
            if (typeof value !== 'string')
                return `must be a duration string, got ${typeof value}`;
        ...
        case 'object':
            if (typeof value !== 'object' || Array.isArray(value))
                return `must be an object, got ${Array.isArray(value) ? 'array' : typeof value}`;
            break;
    }
    return null;
}
 ...
 */
export function validateEntity(meta, typeConfig, isCreate, onlyFields) {
    const errors = [];
    for (const fieldDef of typeConfig.fields) {
        // If onlyFields is specified, skip fields not in the set
        if (onlyFields && !onlyFields.has(fieldDef.key))
            continue;
