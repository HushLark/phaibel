 ...
 */
export function buildEnvelope(body, header, session, identity) {
    // Encrypt the body using X25519 session keys
    const encryptedBody = encryptBody(body, session.contactPublicKey, session.exchangeKeyPair.privateKey);
    ...
    const signature = signMessage(headerForSigning, encryptedBody, identity.identityKeyPair.privateKey);
    // Add signature to header
    const fullHeader = { ...header, signature };
 ...
 */
export function parseEnvelope(base64Envelope) {
 ...
 */
export function decryptEnvelope(header, encryptedBody, session, senderIdentityPublicKey) {
    ...
    const valid = verifySignature(headerForVerification, encryptedBody, signature, senderIdentityPublicKey);
    if (!valid) {
        throw new Error(`PAMP signature verification failed for message ${header.message_id}`);
    }
    // Decrypt body using X25519 session keys
    const body = decryptBody(encryptedBody, session.exchangeKeyPair.privateKey, session.contactPublicKey);
    return {
        header,
        body,
