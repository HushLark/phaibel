...
export function encryptBody(plaintext, recipientPublicKeyBase64, senderPrivateKeyBase64) {
...
export function decryptBody(encryptedPackageBase64, recipientPrivateKeyBase64, senderPublicKeyBase64) {
    const pkg = JSON.parse(Buffer.from(encryptedPackageBase64, 'base64').toString('utf8'));
    ...
    const aesKey = crypto.hkdfSync('sha256', sharedSecret, '', 'pamp-aes-key', 32);
    // AES-256-GCM decrypt
    const decipher = crypto.createDecipheriv('aes-256-gcm', Buffer.from(aesKey), Buffer.from(pkg.nonce, 'base64'));
...
export function signMessage(headerJson, bodyCiphertext, privateKeyPem) {
...
export function verifySignature(headerJson, bodyCiphertext, signature, publicKeyPem) {
    const payload = headerJson + bodyCiphertext;
...
// ── Chain Hash ──────────────────────────────────────────────────────────────
export function computeChainHash(chain) {
