...
import { FCP_VERSION, ProbeRequestSchema, FetchRequestSchema, } from './fcp-types.js';
// ── Manifest ─────────────────────────────────────────────────────────────────
...
// ── Probe handler ───────────────────────────────────────────────────────────
async function handleProbe(res, body) {
    ...
    for (const keyword of req.query.keywords) {
        ...
        for (const r of results) {
            const t = r.node.type;
            if (typeFilter.size > 0 && !typeFilter.has(t))
                continue;
...
// ── Fetch handler ───────────────────────────────────────────────────────────
async function handleFetch(res, body) {
 ...
 */
export async function handleFcpRoute(req, res, url) {
    ...
    try {
        ...
        if (pathname === '/fcp/probe' && method === 'POST') {
            ...
            if (!body)
                return (badRequest(res, 'invalid JSON body'), true);
