...
import { FCP_VERSION, ProbeRequestSchema, ProbeResponseSchema, FetchRequestSchema, FetchResponseSchema, ManifestSchema, } from './fcp-types.js';
...
async function authHeader(source) {
...
// ── Public API ──────────────────────────────────────────────────────────────
export async function probeSource(source, actor, keywords, opts = {}) {
...
export async function fetchFromSource(source, actor, ids, opts = {}) {
...
export async function getManifest(source, timeoutMs = 2000) {
    const headers = await authHeader(source);
 ...
 */
function cxBaseUrl(source) {
    return source.url.replace(/\/fcp\/?$/, '/cx');
}
function assertWritable(source) {
    if (source.mode !== 'readwrite') {
        throw new FcpError(`Source "${source.id}" is read-only`, source.id);
    }
}
export async function listRemoteTypes(source, timeoutMs = 3000) {
    const headers = await authHeader(source);
    const raw = await get(`${cxBaseUrl(source)}/context-types`, headers, timeoutMs);
    return raw.types ?? [];
}
export async function createRemoteNode(source, typeName, data, timeoutMs = 5000) {
    assertWritable(source);
    const headers = await authHeader(source);
    return await post(`${cxBaseUrl(source)}/context-types/${encodeURIComponent(typeName)}`, data, headers, timeoutMs);
}
export async function updateRemoteNode(source, typeName, nodeId, data, timeoutMs = 5000) {
    assertWritable(source);
    const headers = await authHeader(source);
...
export async function deleteRemoteNode(source, typeName, nodeId, timeoutMs = 5000) {
    assertWritable(source);
    const headers = await authHeader(source);
...
async function httpRequest(method, url, body, headers, timeoutMs) {
    const ctrl = new AbortController();
    const timer = setTimeout(() => ctrl.abort(), timeoutMs);
    try {
        const opts = {
