...
import { getVaultRoot, getAgentName, isInterviewComplete, saveProfile, loadState } from '../state/manager.js';
...
export class WebServer {
    ...
    htmlContent = '';
    mobileHtmlContent = '';
    productveHtmlContent = '';
    assistantHtmlContent = '';
    async start(port) {
        // Load HTML at startup
        this.htmlContent = await fs.readFile(path.join(__dirname, 'web-client.html'), 'utf-8');
        try {
            this.mobileHtmlContent = await fs.readFile(path.join(__dirname, 'mobile-client.html'), 'utf-8');
        }
        catch { /* mobile client optional */ }
        try {
            this.productveHtmlContent = await fs.readFile(path.join(__dirname, 'productve.html'), 'utf-8');
        }
        catch { /* productve client optional */ }
        try {
            this.assistantHtmlContent = await fs.readFile(path.join(__dirname, 'Assistant.html'), 'utf-8');
        }
     ...
     */
    broadcastChat(message, category, data) {
        if (!this.wss)
            return;
        ...
        if (req.method === 'POST' && url.pathname === '/api/calendars') {
            try {
                ...
                if (cfg.calendars.some(c => c.id === id)) {
                    res.writeHead(409);
                    res.end(JSON.stringify({ error: `Calendar "${body.name}" already exists` }));
                    return;
                }
            ...
            try {
                ...
                if (sinceIso)
                    nodes = nodes.filter(n => {
                        const upd = (n.meta.updated ?? n.meta.created);
        ...
        if (req.method === 'POST' && url.pathname === '/api/cxf/systems') {
            try {
                ...
                if (!body.id || !body.name || !body.url) {
                    res.writeHead(400);
                    res.end(JSON.stringify({ error: 'id, name, and url required' }));
                    return;
                }
                await addSystem({
                    id: String(body.id),
    ...
    handleWs(ws) {
        ...
        ws.on('message', async (data) => {
            ...
            if (msg.type === 'chat' && msg.message) {
                const onQuestion = (question, options) => {
                    return new Promise((resolve) => {
    ...
    async handleChat(ws, message, onQuestion, chatHistory) {
        let currentChatId;
        try {
            const { response, tokens } = await feralChatHeadless(message, (status) => {
                if (ws.readyState === WebSocket.OPEN) {
...
`;
        ...
        if (body.answers?.location)
            aboutLines.push(`- location: ${body.answers.location}`);
        if (body.answers?.work)
            aboutLines.push(`- work: ${body.answers.work}`);
        if (body.answers?.workGoals)
            aboutLines.push(`- goals: ${body.answers.workGoals}`);
        if (body.answers?.relationships)
            aboutLines.push(`- relationships: ${body.answers.relationships}`);
        if (body.answers?.health)
            aboutLines.push(`- health: ${body.answers.health}`);
        if (body.answers?.growth)
            aboutLines.push(`- growth: ${body.answers.growth}`);
        if (body.answers?.struggles)
            aboutLines.push(`- struggles: ${body.answers.struggles}`);
        if (body.answers?.helpStyle)
            aboutLines.push(`- help_style: ${body.answers.helpStyle}`);
        ...
        if (body.personalCalUrl || body.workCalUrl) {
            try {
                const cfg = await loadCalConfig();
                if (body.personalCalUrl && !cfg.calendars.some((c) => c.id === 'personal')) {
                    cfg.calendars.push({ id: 'personal', name: 'Personal', url: body.personalCalUrl });
                }
                if (body.workCalUrl && !cfg.calendars.some((c) => c.id === 'work')) {
...
// ─────────────────────────────────────────────────────────────────────────────
let _instance = null;
export function setWebServerInstance(server) {
    _instance = server;
}
 ...
 */
export function pushToChat(message, category, data) {
