...
function getOrCreateServer() {
    if (_server)
        return _server;
    _server = new McpServer({ name: 'phaibel', version: '1.0.0' }, {
        capabilities: {
    ...
    // ── Tools ──────────────────────────────────────────────────────────
    _server.tool('chat', 'Send a message to Phaibel and get a response. This goes through the full AI chat pipeline — process selection, execution, and synthesis.', { message: z.string().describe('The user message to process') }, async ({ message }) => {
        try {
    ...
    _server.tool('list_nodes', 'List context nodes of a given type from the Foundation. Returns titles, IDs, and key fields.', {
    ...
    _server.tool('get_node', 'Get a single context node by type and ID. Returns full metadata and content.', {
        ...
        try {
            ...
            if (!entity) {
                return { content: [{ type: 'text', text: `Context node not found: ${type}/${id}` }], isError: true };
            }
    ...
    _server.tool('create_node', 'Create a new context node in the Foundation.', {
        ...
        try {
            ...
            const meta = { id, title, ...fields };
            await writeEntity(filepath, meta, content || '');
            return { content: [{ type: 'text', text: JSON.stringify({ created: true, id, title }) }] };
        }
        catch (err) {
            return { content: [{ type: 'text', text: `Error: ${err instanceof Error ? err.message : String(err)}` }], isError: true };
        }
    });
    _server.tool('search', 'Full-text search across all context nodes in the Foundation.', {
    ...
    _server.tool('list_types', 'List all context types and their schemas.', {}, async () => {
        try {
