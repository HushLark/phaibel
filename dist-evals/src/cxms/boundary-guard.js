...
export class BoundaryViolationError extends Error {
    constructor(filepath, foundationRoot) {
 ...
 */
export async function assertWithinFoundation(filepath) {
 ...
 */
export async function assertWithinRoot(filepath, foundationRoot) {
    ...
    if (storage.realpath) {
        try {
            rootResolved = await storage.realpath(rootResolved);
        }
        catch { /* ignore */ }
        try {
            resolved = await storage.realpath(resolved);
        }
        catch {
            ...
            try {
                resolved = paths.join(await storage.realpath(dir), base);
            }
            catch { /* keep original */ }
        }
    }
