 ...
 */
export async function loadContextTypesFromStore() {
    ...
    try {
        ...
        for (const entry of entries) {
            ...
            try {
                if (!raw)
                    throw new Error('no context file');
 ...
 */
export async function writeContextType(config) {
    const root = await findFoundationRoot();
    if (!root)
        throw new Error('No foundation found');
    const { storage, paths } = getPlatform();
...
`;
    ...
    if (config.defaultTags)
        meta.defaultTags = config.defaultTags;
    if (config.completionField)
        meta.completionField = config.completionField;
    if (config.completionValue)
        meta.completionValue = config.completionValue;
    if (config.spawner)
        meta.spawner = config.spawner;
    if (config.calendarDateField)
        meta.calendarDateField = config.calendarDateField;
 ...
 */
export async function writeMappingIndex(types) {
 ...
 */
export async function writeAllContextTypes(types) {
 ...
 */
export async function removeContextTypeDir(name) {
    const root = await findFoundationRoot();
    if (!root)
        throw new Error('No foundation found');
    const { storage, paths } = getPlatform();
...
function generateExamples(config) {
    ...
    if (config.name === 'task') {
        lines.push('## Example: Buy Groceries', '', '```yaml', 'title: Buy Groceries', 'status: open', 'priority: medium', 'dueDate: 2026-04-10', '```', '');
    }
    else if (config.name === 'event') {
        lines.push('## Example: Team Meeting', '', '```yaml', 'title: Team Meeting', 'startDate: 2026-04-10T14:00:00', 'duration: 1h', 'location: Conference Room A', '```', '');
    }
    else if (config.name === 'note') {
        lines.push('## Example: Project Ideas', '', '```yaml', 'title: Project Ideas', 'tags: [ideas, brainstorm]', '```', '', 'Body content goes here in Markdown.', '');
    }
    else if (config.name === 'person') {
        lines.push('## Example: Jane Smith', '', '```yaml', 'title: Jane Smith', 'lastName: Smith', 'type: colleague', '```', '', '## Example: Cher', '', '```yaml', 'title: Cher', '```', '');
    }
    else {
