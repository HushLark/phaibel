...
import { generateNodeId, nodeFilename } from '../entities/entity.js';
 ...
 */
export async function autoMigrateV4ToV5(root) {
    // Already migrated?
    try {
...
// ── Step 1: Rename root marker ───────────────────────────────────────────────
async function renameRootMarker(root) {
...
async function createV5Directories(root) {
    const dirs = [
        'profiles',
        'context-types',
        'collections',
...
async function migrateProfiles(root) {
...
async function migrateEntityTypes(root) {
    let types;
 ...
 */
async function migrateEntityFiles(root, types) {
    ...
    for (const t of types) {
        if (Object.values(DIR_TO_TYPE).includes(t.name))
            continue;
...
// ── Step 6: Move Feral processes ─────────────────────────────────────────────
async function migrateFeralProcesses(root) {
 ...
 */
async function updateCrossReferences(root, types, idMap) {
    for (const t of types) {
