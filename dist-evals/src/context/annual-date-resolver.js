...
export function resolveFixedDate(mmdd, year) {
...
export function resolveFloatingDate(rule, year) {
    // Named constants
    switch (rule) {
    ...
    if (monthIdx === -1 || weekdayIdx === -1)
        return null;
    if (ordinalStr === 'last')
        return lastWeekday(weekdayIdx, monthIdx, year);
...
export function upcomingAnnualDates(entities, typeFieldMap, windowDays = 365) {
    ...
    for (const entity of entities) {
        ...
        for (const field of fields) {
            if (field.type !== 'date-fixed' && field.type !== 'date-floating')
                continue;
            const value = entity.meta[field.key];
            if (typeof value !== 'string' || !value)
                continue;
            const item = resolveNextOccurrence(entity.id, entity.type, entity.title, field.key, field.label ?? field.key, value, field.type, today, todayYMD, windowDays);
            if (item)
                results.push(item);
        }
    }
...
// ── Helpers ──────────────────────────────────────────────────────────────────
function resolveNextOccurrence(entityId, entityType, title, fieldKey, fieldLabel, rule, recurrenceType, today, todayYMD, windowDays) {
    const year = today.getFullYear();
    const resolve = recurrenceType === 'date-fixed'
        ? (y) => resolveFixedDate(rule, y)
        : (y) => resolveFloatingDate(rule, y);
    for (const y of [year, year + 1]) {
        ...
        if (y === year && dYMD < todayYMD)
            continue; // already passed this year, try next
        break;
    }
    return null;
}
...
function nthWeekday(n, weekday, month, year) {
    const d = new Date(year, month, 1);
    const first = d.getDay();
    let day = 1 + ((weekday - first + 7) % 7) + (n - 1) * 7;
    const result = new Date(year, month, day);
    if (result.getMonth() !== month)
        return null; // n-th doesn't exist this month
    return result;
}
/** Last occurrence of a weekday in a month. */
function lastWeekday(weekday, month, year) {
