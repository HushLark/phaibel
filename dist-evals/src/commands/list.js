...
function printTodos(items) {
    ...
    for (const { file, data } of items) {
        ...
        console.log(`  ${statusIcon} ${chalk.bold.white(title)} ${colorFn(`[${priority}]`)}${due}`);
    }
}
function printEvents(items) {
    for (const { file, data } of items) {
        const title = data.title ?? file.replace('.md', '');
...
function printRecurrences(items) {
    for (const { file, data } of items) {
        const title = data.title ?? file.replace('.md', '');
        ...
        console.log(`  ${type} ${chalk.bold.white(title)}` +
            chalk.gray(`  ${cadence}`) +
            (blackouts > 0 ? chalk.gray(`  ⛔ ${blackouts} blackout${blackouts > 1 ? 's' : ''}`) : ''));
    }
}
function printPeople(items) {
    for (const { file, data } of items) {
        const title = data.title ?? file.replace('.md', '');
...
function printTodonts(items) {
    ...
    for (const { file, data } of items) {
        ...
        if (start && end)
            window = chalk.gray(`${start} → ${end}`);
        else if (start)
            window = chalk.gray(`from ${start}`);
        else if (end)
            window = chalk.gray(`until ${end}`);
        console.log(`  ${icon} ${active ? chalk.bold.red(title) : chalk.gray(title)}  ${window}`);
    }
}
/* ── Helpers ────────────────────────────────────────────────────────── */
