...
import { slugify, generateEntityId, ensureEntityDir, trashEntity, entityFilename, } from '../entities/entity.js';
...
// Re-export from extracted module (avoids pulling commander into Expo)
import { ensureRecurrenceType } from '../entities/recurrence-type.js';
export { ensureRecurrenceType };
// ─────────────────────────────────────────────────────────────────────────────
// FILE I/O
// ─────────────────────────────────────────────────────────────────────────────
async function saveRecurrence(state) {
    ...
    if (state.blackoutWindows.length > 0)
        meta.blackoutWindows = state.blackoutWindows;
...
async function loadAllRecurrences() {
...
async function findRecurrence(name) {
    const all = await loadAllRecurrences();
    const slug = slugify(name);
    return all.find(r => slugify(r.title) === slug ||
        r.title.toLowerCase() === name.toLowerCase()) ?? null;
}
/**
 * Generate concrete entities from recurrence templates.
 * Headless — no console output. Throws on fatal errors.
 */
export async function generateRecurrences(days) {
    await ensureRecurrenceType();
...
async function interactiveCreate() {
    const { title } = await inquirer.prompt([{
            ...
            validate: (i) => i.length > 0 || 'Title is required',
        }]);
    ...
    if (recurrenceType === 'event') {
        const { startTime } = await inquirer.prompt([{
                ...
                default: '09:00',
                validate: (i) => /^\d{2}:\d{2}$/.test(i) || 'Use HH:mm format',
            }]);
        ...
        if (location)
            cadenceDetails.location = location;
    }
    // ── Blackout windows ───────────────────────────────────────────────
    ...
    if (wantBlackouts) {
        while (addingBlackouts) {
            const { start } = await inquirer.prompt([{
                    ...
                    validate: (i) => /^\d{4}-\d{2}-\d{2}$/.test(i) || 'Use YYYY-MM-DD',
                }]);
...
// EDIT
// ─────────────────────────────────────────────────────────────────────────────
async function editRecurrence(name) {
...
// DELETE
// ─────────────────────────────────────────────────────────────────────────────
async function deleteRecurrence(name) {
    const rec = await findRecurrence(name);
    if (!rec) {
        console.log(chalk.red(`\nRecurrence "${name}" not found.`));
        return;
    ...
    if (confirm && rec.filepath) {
        ...
        const recId = matter(raw).data.id ?? path.basename(rec.filepath, '.md');
