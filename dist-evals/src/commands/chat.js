...
import { updateProfile, validateScores, invalidateProfileCache } from '../personality/big-five.js';
...
import { runWithTokenTracker } from '../llm/token-usage.js';
// ─────────────────────────────────────────────────────────────────────────────
...
function formatHistoryBlock(history) {
    if (history.length === 0)
        return '';
    const lines = history.map(h => h.role === 'user' ? `User: ${h.content}` : `Assistant: ${h.content}`);
...
export async function feralChatHeadless(userInput, onStatus, onProcess, onQuestion, onChatId, history, platform, clientHints) {
    ...
    status('Thinking…');
    try {
        const { result: response, tokens } = await runWithTokenTracker(() => _feralChatHeadlessInner(userInput, status, onProcess, onQuestion, logger, chatId, history ?? [], platform, clientHints));
...
async function _feralChatHeadlessInner(userInput, status, onProcess, onQuestion, logger, chatId, history, platform, clientHints) {
    ...
    const filteredNodes = filterCatalogNodes(allNodes.filter(n => !n.key.startsWith('speak_')), relevance.relevantTypes, entityTypes);
    ...
    if (relevance.keywords.length > 0) {
        try {
            ...
            pdContext = await runProgressiveDisclosure(llm, userInput, vaultContext, relevance.keywords);
            if (pdContext) {
    ...
    try {
        ...
        const cdResult = await runCapabilityDiscovery(llm, userInput, pdContext, skillMetas, runtime.catalog, runtime.nodeCodeFactory);
        ...
        if (cdResult.selectedSkill) {
            ...
            if (matchedMeta.scriptNames.length > 0) {
                ...
                if (script) {
                    ...
                    const skillProcess = hydrateProcess(script.process);
                    ...
                    if (success) {
                        ...
                        const finalResponse = await synthesizeResponse(llm, userInput, `Skill "${matchedMeta.name}"`, [scrubbedResult], [], vaultContext, history, chatId, clientHints);
...
Return ONLY the JSON object, no markdown fences.`,
        ...
        try {
            ...
            if (matchResult.action === 'reuse' && matchResult.process_key) {
                ...
                // Synthesize response
                const finalResponse = await synthesizeResponse(llm, userInput, matchResult.reasoning, [scrubbedResult], [], vaultContext, history, chatId, clientHints);
                await logger.log('response', { response: finalResponse });
...
Return ONLY the JSON object, no markdown fences.`,
        ...
        try {
            ...
            const processContext = (processDesign.process.context ?? {});
...
Return ONLY the JSON object, no markdown fences.`,
    ...
    const finalResponse = await synthesizeResponse(llm, userInput, nodeSelection.reasoning, allResults, nodesUsed, vaultContext, history, chatId, clientHints);
    await logger.log('response', { response: finalResponse });
    // Fire-and-forget execution log
...
// SYNTHESIS HELPER
// ─────────────────────────────────────────────────────────────────────────────
async function synthesizeResponse(llm, userInput, reasoning, results, nodesUsed, vaultContext, history = [], chatId, clientHints) {
...
You MUST return valid JSON with this structure:
{
    ...
    const rawResponse = await llm.chat([{ role: 'user', content: synthesisPrompt }], {
    ...
    try {
        ...
        responseText = parsed.response || rawResponse.trim();
        // Extract and store personality observation (fire-and-forget)
        const obs = parsed.personality_observation;
        if (obs) {
        ...
        const assumedNodes = parsed.assumed_nodes;
 ...
 */
async function createAssumedNodes(nodes) {
    for (const node of nodes) {
        if (!node.contextType || !node.title)
            continue;
        ...
        const meta = {
            ...
            created: now,
            tags: node.tags || typeConfig.defaultTags || [],
            source: 'assumed',
        };
 ...
 */
async function judgeResponse(userInput, responseText, chatId) {
    try {
