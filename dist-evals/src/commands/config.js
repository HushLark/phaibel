...
import { loadConfig, setApiKey, setCapabilityModel, removeCapabilityOverride, getApiKey, getEffectiveConfig, getConfiguredProviders, PROVIDER_MODELS, } from '../config.js';
...
configCommand
    ...
    for (const cap of LLM_CAPABILITIES) {
        ...
        if (mapping) {
            ...
            console.log(`  ${chalk.bold(cap.padEnd(12))} ${chalk.gray(capDescriptions[cap].padEnd(18))} ${mapping.provider}/${mapping.model}${overrideTag}`);
        }
        else {
            const hint = cap === 'embed' ? chalk.gray(' (requires OpenAI — Anthropic has no embedding model)') : '';
            console.log(`  ${chalk.bold(cap.padEnd(12))} ${chalk.gray(capDescriptions[cap].padEnd(18))} ${chalk.red('no provider configured')}${hint}`);
        }
    }
    console.log('');
...
configCommand
    ...
    .action(async (capability, provider, model) => {
    if (!LLM_CAPABILITIES.includes(capability)) {
        console.log(chalk.red(`Unknown capability: ${capability}`));
        console.log(chalk.gray(`Valid capabilities: ${LLM_CAPABILITIES.join(', ')}`));
        return;
    }
    await setCapabilityModel(capability, provider, model);
...
configCommand
    ...
    await removeCapabilityOverride(capability);
