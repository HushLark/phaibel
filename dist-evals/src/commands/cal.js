...
import { ensureEntityDir, writeEntity, listEntities, createEntityMeta, entityFilename, } from '../entities/entity.js';
...
export async function loadCalConfig() {
    try {
...
export async function saveCalConfig(cfg) {
 ...
 */
async function syncOneCalendar(cal, daysAhead) {
    const now = new Date();
    ...
    for (const ent of existingEntities) {
        const uid = ent.meta.calendarUid;
        const entCalId = ent.meta.calendarId;
    ...
    for (const ev of upcoming) {
        ...
        if (existing) {
            ...
            const changed = meta.startDate !== ev.startDate ||
