...
// Handles any user-defined entity type: `phaibel entity <type> [list|add|remove|show]`
...
import { createEntityMeta, ensureEntityDir, writeEntity, findEntityByTitle, trashEntity, listEntities, entityFilename, } from '../entities/entity.js';
...
async function promptForField(field) {
    ...
    const { value } = await inquirer.prompt([{
            ...
            validate: field.required
                ? (v) => v.trim().length > 0 || `${field.label ?? field.key} is required`
                : undefined,
        }]);
    if (!value.trim() && !field.required)
        return undefined;
    if (field.type === 'number')
        return Number(value.trim()) || undefined;
    return value.trim();
}
// ─────────────────────────────────────────────────────────────────────────────
// ACTIONS
// ─────────────────────────────────────────────────────────────────────────────
async function addEntity(typeName, titleArg) {
    const typeConfig = await getEntityType(typeName);
    if (!typeConfig) {
        console.log(chalk.red(`\n  Unknown entity type: "${typeName}"`));
        console.log(chalk.gray('  Run: phaibel type list'));
        return;
    }
    ...
    const fullMeta = { ...meta, ...fieldValues };
    await writeEntity(filepath, fullMeta, '');
    // Update index
    const index = getEntityIndex();
    if (index.isBuilt)
        await index.addOrUpdate(typeName, id, title, filepath);
    console.log(chalk.green(`\n  ✓ ${capitalize(typeName)} "${title}" saved.`));
}
async function listEntity(typeName) {
    const typeConfig = await getEntityType(typeName);
    if (!typeConfig) {
        console.log(chalk.red(`\n  Unknown entity type: "${typeName}"`));
        return;
...
async function removeEntity(typeName, titleArg) {
    const typeConfig = await getEntityType(typeName);
    if (!typeConfig) {
        console.log(chalk.red(`\n  Unknown entity type: "${typeName}"`));
        return;
    ...
    if (index.isBuilt) {
        ...
        if (id)
            index.remove(typeName, id);
    }
    console.log(chalk.green(`\n  ✓ ${capitalize(typeName)} "${title}" removed.\n`));
}
async function showEntity(typeName, titleArg) {
    const typeConfig = await getEntityType(typeName);
    if (!typeConfig) {
        console.log(chalk.red(`\n  Unknown entity type: "${typeName}"`));
        return;
...
export const entityCommand = new Command('entity')
    ...
    .action(async (type, action, title) => {
    switch (action) {
        case 'add':
            await addEntity(type, title);
            break;
        case 'list':
            await listEntity(type);
            break;
        case 'remove':
        case 'rm':
            await removeEntity(type, title);
            break;
        case 'show':
            await showEntity(type, title);
            break;
        default:
