...
import { loadSourceRegistry, saveSourceRegistry, addSource, removeSource, getManifest, probeSource, fetchFromSource, probeAll, } from '../federation/index.js';
...
fcpCommand
    ...
    for (const s of reg.sources) {
        ...
        if (s.scopes.length > 0)
            console.log(chalk.gray(`      scopes: ${s.scopes.join(', ')}`));
        console.log('');
    }
}));
// ── add ──────────────────────────────────────────────────────────────────────
fcpCommand
    ...
    .action(exitAfter(async (id, opts) => {
...
fcpCommand
    ...
    try {
        ...
        if (manifest.contact)
            console.log(chalk.gray(`  contact: ${manifest.contact}`));
        console.log('');
    }
    catch (err) {
...
fcpCommand
    ...
    .action(exitAfter(async (keywords, opts) => {
    if (opts.source) {
        const reg = await loadSourceRegistry();
...
fcpCommand
    ...
    .action(exitAfter(async (sourceId, ids, opts) => {
    ...
    try {
        ...
        for (const n of resp.nodes) {
            ...
            if (n.summary)
                console.log(chalk.gray(`      ${n.summary}`));
            if (n.body) {
