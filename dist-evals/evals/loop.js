...
function analyze(current, baseline) {
    if (!baseline) {
        return {
            improved: true,
    ...
    for (const curr of current.scenarios) {
        ...
        if (!prev) {
            if (curr.passed)
                improvements.push(`+ NEW ${curr.scenarioId}: PASS`);
            else
                unchanged.push(`  NEW ${curr.scenarioId}: FAIL`);
            continue;
        }
...
// COMMIT
// ─────────────────────────────────────────────────────────────────────────────
async function autoCommit(label, analysis, result, resultFile) {
