 ...
 */
export async function createEvalVault(seed, vaultContext) {
    ...
    // Entity types config
    await fs.writeFile(path.join(configDir, 'entity-types.json'), JSON.stringify({ version: 1, entityTypes: EVAL_ENTITY_TYPES }, null, 2));
    ...
    if (seed && seed.length > 0) {
        for (const s of seed) {
            ...
            const filepath = path.join(dir, entityFilename(s.title, meta.id));
            await writeEntity(filepath, meta, s.body ?? '');
        }
    }
    return vaultDir;
}
/** Map entity type name to directory name. */
function getEntityDir(entityType) {
 ...
 */
export async function destroyEvalVault() {
    if (originalCwd) {
