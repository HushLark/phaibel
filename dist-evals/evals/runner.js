 ...
 */
async function runScenario(scenario, modelOverrides) {
    ...
    try {
        ...
        try {
            responseText = await Promise.race([
                feralChatHeadless(scenario.userInput, () => { }, // onStatus: no-op
                ...
                scenario.history).then(r => r.response),
                new Promise((_, reject) => setTimeout(() => reject(new Error(`Scenario timed out after ${timeoutMs}ms`)), timeoutMs)),
            ]);
        }
        catch (err) {
 ...
 */
export async function runEval(scenarios, config) {
    const results = [];
