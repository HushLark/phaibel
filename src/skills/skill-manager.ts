...
import type { SkillFrontmatter, SkillMeta, SkillManifest, SkillScript } from './types.js';
...
async function skillMetaFromDir(directory: string): Promise<SkillMeta | null> {
 ...
 */
export async function loadSkillManifest(meta: SkillMeta): Promise<SkillManifest> {
    const { paths } = getPlatform();
 ...
 */
export async function loadSkillScript(meta: SkillMeta, scriptName?: string): Promise<SkillScript | null> {
    const { paths } = getPlatform();
    ...
    try {
        ...
        const process = JSON.parse(raw) as Record<string, unknown>;
 ...
 */
export async function loadAllSkillScripts(meta: SkillMeta): Promise<SkillScript[]> {
