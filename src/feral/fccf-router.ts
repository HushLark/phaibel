...
import {
    ...
    serverError,
    readBody,
} from '../cxms/problem-details.js';
import { bootstrapFeral, type FeralRuntime } from './bootstrap.js';
 ...
 */
export async function handleFccfRoute(
...
async function handleListProcesses(res: http.ServerResponse): Promise<boolean> {
    const feral = await getFeral();
...
async function handleRunAdhoc(req: http.IncomingMessage, res: http.ServerResponse): Promise<boolean> {
    const raw = await readBody(req);
    let body: { process?: Record<string, unknown>; context?: Record<string, unknown> };
    try {
        body = JSON.parse(raw || '{}');
    } catch {
    ...
    try {
        ...
        const process = hydrateProcess(body.process as unknown as ProcessConfigJson);
...
async function handleRunStored(
    req: http.IncomingMessage,
    res: http.ServerResponse,
    processKey: string,
): Promise<boolean> {
    ...
    try {
        if (raw) contextValues = JSON.parse(raw);
    } catch {
        badRequest(res, 'Invalid JSON body');
        return true;
    }
