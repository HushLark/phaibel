...
import type { FederatedRelevance } from '../../../federation/federator.js';
...
export class FcpFetchNodeCode extends AbstractNodeCode {
    ...
    async process(context: Context): Promise<Result> {
        const probeKey = (this.getOptionalConfigValue('probe_context_path') as string | null) ?? 'fcp_probe';
        const contextPath = (this.getOptionalConfigValue('context_path') as string | null) ?? 'fcp_nodes';
        const detail = ((this.getOptionalConfigValue('detail') as string | null) ?? 'full') as 'summary' | 'full';
        const maxPerSource = parseInt((this.getOptionalConfigValue('max_per_source') as string | null) ?? '5', 10);
        const minScore = parseFloat((this.getOptionalConfigValue('min_score') as string | null) ?? '0.5');
        const purpose = this.getOptionalConfigValue('purpose') as string | null ?? undefined;

        const probeResult = context.get(probeKey) as FederatedRelevance | null;
        if (!probeResult) {
