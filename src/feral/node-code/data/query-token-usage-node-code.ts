...
export class QueryTokenUsageNodeCode extends AbstractNodeCode {
    ...
    constructor() {
        super(
            'query_token_usage',
            'Query Token Usage',
            'Queries LLM token usage by model or all models over the last N days.',
            NodeCodeCategory.DATA,
        );
    }

    async process(context: Context): Promise<Result> {
        const model = this.getRequiredConfigValue('model', 'all') as string;
        const days = Math.min(30, parseInt(this.getRequiredConfigValue('days', '30') as string, 10) || 30);
        ...
        try {
            ...
            return this.result(
                ResultStatus.OK,
                `Queried ${days} days of usage for ${model}: ${totalTokens.toLocaleString()} tokens across ${totalCalls} calls.`,
            );
        } catch (error) {
