...
import { SourceRegistrySchema, type SourceRegistry, type SourceConfig } from './fcp-types.js';
import { debug } from '../utils/debug.js';

const DEFAULT_REGISTRY: SourceRegistry = { sources: [] };
...
export async function removeSource(id: string): Promise<boolean> {
    ...
    if (reg.sources.length === before) return false;
    await saveSourceRegistry(reg);
    return true;
}
