...
import type { Catalog } from '../feral/catalog/catalog.js';
import type { NodeCodeFactory } from '../feral/node-code/node-code-factory.js';
...
interface CapabilityDecision {
    needsSkillDetail: string[];      // skill names to load full manifest for
    needsNodeDetail: string[];       // catalog node keys to load config/result docs for
    selectedSkill: string | null;    // skill name to run directly (or null)
    selectedScript: string | null;   // script name within the skill (or null = first)
    hasEnoughContext: boolean;
    capabilitySummary?: string;
}

export interface CapabilityDiscoveryResult {
    /** Enriched capability context to inject into Step 1 and Step 2 prompts. */
    contextSnapshot: string;
    /** If set, skip the custom pipeline and run this skill directly. */
    selectedSkill: SkillMeta | null;
    /** Which script within the selected skill to run (null = first). */
...
function formatSkillMetas(metas: SkillMeta[]): string {
    if (metas.length === 0) return '(no skills installed)';
    return metas.map(m => {
...
function formatCatalogSummary(catalog: Catalog): string {
...
async function fetchSkillDetail(meta: SkillMeta, scriptName?: string): Promise<string> {
    ...
    if (sName) {
        ...
        if (script) {
            ...
            const nodes = (script.process.nodes ?? []) as Array<{
                key: string;
...
function fetchNodeDetail(nodeKey: string, catalog: Catalog, nodeCodeFactory: NodeCodeFactory): string {
    let catalogNode;
    try {
    ...
    const lines: string[] = [`--- NODE: ${nodeKey} (${catalogNode.nodeCodeKey}) ---`];
 ...
 */
export async function runCapabilityDiscovery(
    llm: LLMProvider,
    userInput: string,
    entityContext: string,
    skillMetas: SkillMeta[],
    catalog: Catalog,
    nodeCodeFactory: NodeCodeFactory,
): Promise<CapabilityDiscoveryResult> {
    const skillTier1 = formatSkillMetas(skillMetas);
    const catalogTier1 = formatCatalogSummary(catalog);

    const collectedSkillDetails: string[] = [];
    const collectedNodeDetails: string[] = [];
  ...
  "capabilitySummary": "brief note on what you've determined"
        ...
        try {
            const raw = await llm.chat(
                ...
                { systemPrompt: 'You are a capability planner for Phaibel. Respond with JSON only.', temperature: 0.1 },
            );
            const json = raw.match(/\{[\s\S]*\}/)?.[0] ?? raw;
            decision = JSON.parse(json) as CapabilityDecision;
        } catch (err) {
            debug('capability-discovery', `Failed to parse decision at iteration ${iteration}: ${err}`);
            break;
        }

        if (decision.capabilitySummary) capabilitySummary = decision.capabilitySummary;

        // Fetch requested skill details
        for (const name of (decision.needsSkillDetail ?? [])) {
            if (fetchedSkills.has(name)) continue;
        ...
        for (const key of (decision.needsNodeDetail ?? [])) {
            if (fetchedNodes.has(key)) continue;
...
function buildSnapshot(
    ...
    const parts: string[] = [];
    if (summary) parts.push(`CAPABILITY SUMMARY: ${summary}`);
    if (skillDetails.length > 0) parts.push(`SKILL DETAILS:\n${skillDetails.join('\n\n')}`);
    if (nodeDetails.length > 0) parts.push(`NODE DOCUMENTATION:\n${nodeDetails.join('\n\n')}`);
    return parts.join('\n\n');
}
