...
export type ContextScope =
    ...
    | { type: 'branch-full'; branches: string[] }
    | { type: 'leaves'; keys: string[] }
    | { type: 'cross'; branches: string[]; query?: string };

export interface ContextTreeTrunk {
    vaultContext: string;
    globals: Record<string, string>;
    stats: { totalEntities: number; entityTypes: string[] };
}

export interface ContextTreeField {
    key: string;
    type: string;
    values?: string[];
}
...
export interface ContextTreeBranch {
    entityType: string;
    plural: string;
    description: string;
    fields: ContextTreeField[];
    subdirectoryContext: string;
    leafCount: number;
    leaves: ContextTreeLeaf[];
}
...
export interface ContextTree {
    trunk: ContextTreeTrunk;
    branches: ContextTreeBranch[];
...
function fieldToTreeField(f: FieldDef): ContextTreeField {
    const tf: ContextTreeField = { key: f.key, type: f.type };
    if (f.type === 'enum' && f.values) tf.values = f.values;
    return tf;
}

function indexNodeToLeaf(node: IndexNode, includeContent?: string): ContextTreeLeaf {
    return {
 ...
 */
async function buildTrunk(globals: Record<string, string>): Promise<ContextTreeTrunk> {
 ...
 */
function filterEdges(allEdges: IndexEdge[], includedKeys: Set<string>): ContextTreeEdge[] {
    return allEdges
...
export async function buildContextTree(
    ...
    let branches: ContextTreeBranch[] = [];
    const includedLeafKeys = new Set<string>();

    switch (scope.type) {
        case 'trunk': {
            // All branches with metadata only (no leaves)
            branches = await Promise.all(
                entityTypes.map(t => buildBranch(t, 'none'))
            );
            break;
        }
        ...
        case 'branch': {
            ...
            branches = await Promise.all(
                entityTypes.map(t =>
                    buildBranch(t, scope.branches.includes(t.name) ? 'summary' : 'none')
                )
            );
            break;
        }
        ...
        case 'branch-full': {
            ...
            branches = await Promise.all(
                entityTypes.map(t =>
                    buildBranch(t, scope.branches.includes(t.name) ? 'full' : 'none')
                )
            );
            break;
        }
        ...
        case 'leaves': {
            ...
            for (const key of scope.keys) {
                ...
                if (node) leafTypes.add(node.type);
            }
            // Build branches: types with requested leaves get summary, others none
            branches = await Promise.all(
                entityTypes.map(t => buildBranch(t, leafTypes.has(t.name) ? 'summary' : 'none'))
            );
            break;
        }
 ...
 */
export async function expandContextTree(
    tree: ContextTree,
