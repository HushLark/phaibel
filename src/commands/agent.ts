...
agentCommand
    ...
    .action(async (name: string, opts: { url: string; description?: string; headers?: string[] }) => {
    ...
    .action(async (name?: string) => {
        ...
        for (const agent of targets) {
            try {
                ...
                const card = await response.json() as { name: string; skills?: Array<{ id: string; name: string }> };
